 ...
 * Copyright (c) 2021-2022 梁家炜 all rights reserved.
 * 版权所有，侵权必究！
 */
    ...
    public String checkDone(Long[] ids) {
        ...
        if (!CollectionUtils.isEmpty(orders)) {
            ...
            if (StringUtils.isEmpty(errorOrderNos)) {
                //订单状态正常 可以执行管理员沟通卖家完成操作 修改订单状态和更新时间
                ...
                if (errorOrderNos.length() > 0 && errorOrderNos.length() < 100) {
                    ...
                    return "你选择了太多状态不是支付成功的订单，无法执行管理员沟通卖家完成操作";
                }
            }
        }
        //未查询到数据 返回错误提示
    ...
    public String checkOut(Long[] ids) {
        ...
        if (!CollectionUtils.isEmpty(orders)) {
            ...
            if (StringUtils.isEmpty(errorOrderNos)) {
                ...
                //订单此时不可执行出库操作
                if (errorOrderNos.length() > 0 && errorOrderNos.length() < 100) {
                    return errorOrderNos + "订单的状态不是支付成功或管理员沟通卖家完成无法执行出库操作";
                } else {
                    return "你选择了太多状态不是支付成功或管理员沟通卖家完成的订单，无法执行出库操作";
                }
            }
        }
        //未查询到数据 返回错误提示
