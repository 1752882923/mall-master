$(function () {
    ...
    function orderStatusFormatter(cellvalue) {
        //订单状态:0.待支付 1.已支付 2.管理员沟通卖家完成 3:出库成功 4.交易成功 -1.手动关闭 -2.超时关闭 -3.商家关闭
        if (cellvalue == 0) {
            return "待支付";
        }
        if (cellvalue == 1) {
...
function orderCheckDone() {
    ...
    if (orderNos.length > 0 & orderNos.length < 100) {
        swal(orderNos + "订单的状态不是支付成功无法执行管理员沟通卖家完成操作", {
            icon: "error",
        });
        return;
    }
    if (orderNos.length >= 100) {
        swal("你选择了太多状态不是支付成功的订单，无法执行管理员沟通卖家完成操作", {
            icon: "error",
        });
        return;
    }
...
function orderCheckOut() {
    ...
    for (i = 0; i < ids.length; i++) {
        var rowData = $("#jqGrid").jqGrid("getRowData", ids[i]);
        if (rowData.orderStatus != '已支付' && rowData.orderStatus != '管理员沟通卖家完成') {
            orderNos += rowData.orderNo + " ";
        }
    }
    if (orderNos.length > 0 & orderNos.length < 100) {
        swal(orderNos + "订单的状态不是支付成功或管理员沟通卖家完成无法执行出库操作", {
            icon: "error",
        });
        return;
    }
    if (orderNos.length >= 100) {
        swal("你选择了太多状态不是支付成功或管理员沟通卖家完成的订单，无法执行出库操作", {
            icon: "error",
        });
        return;
    }
